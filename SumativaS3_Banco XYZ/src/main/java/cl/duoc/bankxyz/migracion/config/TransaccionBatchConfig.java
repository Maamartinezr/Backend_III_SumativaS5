...
public class TransaccionBatchConfig {
    ...
    @Bean
    public BatchStepListener transaccionStepListener(@Value("${bankxyz.batch.thread-count:3}") int threadCount,
                                                     @Value("${bankxyz.batch.chunk-size:5}") int chunkSize,
                                                     @Value("${bankxyz.batch.config-label:hilos-3-chunk-5}") String configLabel,
                                                     @Value("${bankxyz.evidencias.directorio:target/evidencias}") String evidenciaDir) {
        return new BatchStepListener(threadCount, chunkSize, configLabel, evidenciaDir);
    }
    ...
    public Step transaccionStep(JobRepository jobRepository,
                                 ...
                                 @Qualifier("transaccionStepListener") BatchStepListener transaccionStepListener,
        ...
        return new StepBuilder("transaccionStep", jobRepository)
                ...
                // Escalamiento: hilos y chunks configurables para benchmark
                .taskExecutor(batchTaskExecutor)
                .faultTolerant()
